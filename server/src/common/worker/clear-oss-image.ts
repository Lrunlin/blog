...
async function clearOSSImage() {
  ...
  // bucketManager.listPrefix(bucket, options, async (err, respBody, respInfo) => {
  //   if (err) {
  ...
  //   /** 上传超过七天的图片列表*/
  //   let imageList: string[] = respBody.items.map((item: any) => item.key.replace("article/", ""));
  ...
  //   /** 需要删除的图片列表*/
  //   let removeImageList = images
  //     .filter(item => !imageList.includes(item))
