  ...
  percentages.forEach(async (weight, index) => {
    condition.forEach(async (item) => {
      await DB.Article.findOne({
        ...
        if (row) {
          let value = (row as Article)[item.key as keyof ArticleAttributes] as
            | string
            | number;
    ...
    (sort) => {
      ...
      for (key in option) {
        ...
        let _score =
          ...
          (condition.find((item) => item.key == key) as (typeof condition)[0])
            .importance[sort as "recommend" | "newest" | "hottest"] *
