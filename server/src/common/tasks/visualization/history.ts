...
async function setVisitsData() {
  ...
  for (const key of await redis.keys("history-article*")) {
    ...
    _articleRanking[articleID]
      ? _articleRanking[articleID]++
      : (_articleRanking[articleID] = 1);
    /** 统计访问来源和七日内全站文章访问量*/
    await redis
      .lrange(key, 0, -1)
  ...
  ipHistory.forEach((item) => {
    let visitsItem = visits.find(
      (_item) => item.time.substring(5) == _item.time,
    );
    if (visitsItem) {
  ...
  articleRanking = await DB.Article.findAll({
    ...
    .then((rows) =>
      rows.map((item, index) => ({
        ...item,
        view_count: articleIDs[index].viewCount,
      })),
    )
    .catch(() => null);
