...
const schema = Joi.object({
  email: Joi.string()
    ...
    .required()
    .email()
    .error(new Error("邮箱为5-30的字符串")),
  name: Joi.string()
    .min(1)
    .max(12)
    .required()
    .error(new Error("昵称为1-12的字符串")),
  password: Joi.string()
    .min(1)
    .max(16)
    .required()
...
router.post("/email/link", validator(schema), async (ctx) => {
  ...
  let { count: existEmailCount } = await DB.User.findAndCountAll({
    where: { email: email },
  });
  ...
  if (existEmailCount) {
    ...
    ctx.body = {
      success: false,
      message: `已经存在绑定该邮箱的用户，请使用找回密码功能`,
    };
    return;
  }
  ...
  if ((await hasKey(createKey(email))) && minute > 5) {
    ...
    ctx.body = {
      success: false,
      message: `链接已发送并剩余时间:${minute}分钟,请前往邮箱点击链接`,
    };
    return;
  }
  ...
  const content = ejs.render(
    fs.readFileSync("src/views/logn-up.ejs").toString(),
    {
  ...
  await sendEmail({
    target: email,
    subject: `${process.env.SITE_NAME}-注册`,
    content,
  })
    .then((res) => {
      ctx.body = {
        success: true,
        message: "激活邮件发送成功，请在15分钟内点击",
      };
