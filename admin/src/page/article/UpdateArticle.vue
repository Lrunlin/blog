<template>
    ...
    <el-result icon="warning" title="未找到对应文章" sub-title="没有找到对应的文章" v-if="isSuccess === false">
        <template #extra>
...
function update() {
    ...
    axios.put(`/article/${route.params.id}`, { router: data.router, title: data.title, type: data.type, article: data.article, oldHtml: data.oldHtml }).then(res => {
        if (res.data.success) {
