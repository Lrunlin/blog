    ...
    <el-tabs type="card" v-model="activeTab" :before-leave="beforeLeave">
        ...
        <el-tab-pane label="发布" :disabled="!hasReleaseRecord">
            <el-result icon="success" title="文章发布成功" :sub-title="`文章标题:${data.title}`">
                <template #extra>
                    <el-button type="primary" size="medium" @click="link" v-if="data.router">去看看</el-button>
                    <el-button type="primary" size="medium" @click="router.push('/article')">返回列表</el-button>
                </template>
            </el-result>
        </el-tab-pane>
    </el-tabs>
...
function release() {
    ...
    axios.post('/article', { router: data.router, title: data.title, type: data.type, article: data.article }).then(res => {
        if (res.data.success) {
...
function beforeLeave(activeName, oldActiveName) {
    if (oldActiveName == '0') {
        ...
        if (data.router && !/^[A-Za-z0-9]+$/.test(data.router)) {
            ElMessage.error('文章路由只能由数字或字母组成');
            return false;
        }
    };
    if (oldActiveName == '1') {
...
function link() {
    let path = process.env.NODE_ENV === "production" ? "https://blogweb.cn/article" : "http://localhost:5678/article";
