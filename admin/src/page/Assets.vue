    ...
    <el-table :data="tableData" style="width: 100%">
        <el-table-column label="图片" width="220">
            <template v-slot="scope">
                <el-image
                    style="width: 100px;"
                    :src="`${assets}/image/${scope.row.name}${scope.row.id}`"
                    :previewSrcList="[`${assets}/image/${scope.row.name}`]"
                    lazy
                ></el-image>
            </template>
        </el-table-column>
        <el-table-column label="类型" width="80">
            <template v-slot="scope">
                <el-icon v-if="scope.row.name.includes('@')">
                    <UserFilled />
                </el-icon>
                <el-icon v-else>
                    <Notebook />
        ...
        <el-table-column label="删除" width="120">
            <template v-slot="scope">
                <el-popconfirm title="确定删除该图片？" @confirm="remove(scope.row.name, scope.$index)">
                    <template #reference>
...
function remove(name, index) {
    axios.delete(`${assets}/assets`, { params: { images: [name] } }).then(res => {
        if (res.data.success) {
